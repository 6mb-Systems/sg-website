              ...
              <FadeIn key={index} direction="up" delay={index * 0.05}>
                <div className="relative h-full overflow-hidden rounded-xl border border-gray-200">
                  ...
                  <svg className="absolute inset-0 h-full w-full opacity-30" aria-hidden>
                    <defs>
                      <pattern id={`hive-feature-hex-${index}`} x="0" y="0" width="60" height="34.64" patternUnits="userSpaceOnUse" patternTransform="scale(2)">
                        <path d="M0 17.32L10 0H30L40 17.32L30 34.64H10L0 17.32Z M40 17.32H60" fill="none" stroke="#d1d5db" strokeWidth="0.55" />
                      </pattern>
                    </defs>
                    <rect width="100%" height="100%" fill={`url(#hive-feature-hex-${index})`} />
                  </svg>
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-2">
            {STRATEGIC_ADVANTAGES.map((item, index) => (
              <FadeIn key={index} direction="up" delay={index * 0.08}>
                <div className="relative h-full overflow-hidden rounded-xl border border-gray-200">
                  <div className="absolute inset-0 bg-gradient-to-br from-white to-brand-blue-50/40" aria-hidden />
                  <svg className="absolute inset-0 h-full w-full opacity-30" aria-hidden>
                    <defs>
                      <pattern id={`hive-strategy-hex-${index}`} x="0" y="0" width="60" height="34.64" patternUnits="userSpaceOnUse" patternTransform="scale(2)">
                        <path d="M0 17.32L10 0H30L40 17.32L30 34.64H10L0 17.32Z M40 17.32H60" fill="none" stroke="#d1d5db" strokeWidth="0.55" />
                      </pattern>
                    </defs>
                    <rect width="100%" height="100%" fill={`url(#hive-strategy-hex-${index})`} />
                  </svg>
                  <div className="relative z-10 flex h-full items-start gap-3 p-6">
